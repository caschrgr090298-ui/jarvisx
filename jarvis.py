...
2. Convert the raw coordinates/keys into LOGICAL named steps (e.g. "Click the Spotify search 3. Write an ADAPTIVE strategy — steps that describe the INTENT so JARVIS can figure out how t4. Note any steps that look inefficient and suggest the better way.
...
def screen_aware_ask(user_command: str) -> str:
 ...
 f"Use what you see on screen to give the most accurate response o )
 }
 ]
 }]
 )
...
def execute_action(act_dict: dict) -> str:
 ...
 webbrowser.open(f"https://open.spotify.com/search/{urllib.parse.quote(query)} return f"Opened Spotify web search for {query}."
 ...
 query = args.get("query", "")
 webbrowser.open(f"https://www.youtube.com/results?search_query={urllib.parse.quot time.sleep(3)
...
When user asks to generate, create, draw, or make an image/picture/photo:
 → action "generate_image", args {{"prompt":"detailed description","filename":"optional_name The prompt should be detailed and descriptive for best results.
 Example: "generate_image" with prompt "a futuristic city at night with neon lights and flyi The image auto-saves as PNG to Desktop and opens in Windows Photos automatically.
════════════════════════════════════════════════════
ALL ACTIONS
════════════════════════════════════════════════════
...
stop_recording {{"name":"open spotify"}} ← when user says "stop", "done", "save that aexecute_skill {{"name":"open spotify","context":""}} ← when user says "do that thing"correct_skill {{"name":"open spotify","correction":"you opened the wrong playlist"}} list_skills {{}} ← when user asks what you know how to do
IMPORTANT CORRECTION HANDLING:
- If user says "[skill name] was wrong" or "you did [skill] wrong" or "that's not how to [ski → use correct_skill action with their explanation as the correction field
...
teach_command {{"phrase":"open my report","action_obj":{{"action":"open_file","args":{{enable_startup {{}}
disable_startup {{}}
